...
- **Cart item translation at session-load**: hooks `woocommerce_get_cart_item_from_session` and `woocommerce_add_cart_item` to translate the cart item's `data` (WC_Product object) and `variation` (attribute slug map) to the current language. Verified against WC 10.7 source: `CartItemSchema` reads `name`/`description`/`sku` directly from the product object, so swapping the product object is the only point at which translation can be injected for Block cart.
- **Cart line identity preserved**: `product_id`, `variation_id`, and `data_hash` are deliberately NOT mutated. The cart line's KEY (used by `WC_Cart::generate_cart_id()` and `find_product_in_cart()`) must stay anchored to the original adding-language IDs to keep dedup working across language switches. Earlier (now-reverted) versions also mutated those fields, which broke dedup and required a `data_hash` recompute hack to survive session round-trip; the no-mutation design is simpler and correct.
- **Cart-line dedup uses canonical IDs**: the dedup readers in `addToCart` and `dedupeStoreApiAdd` consult `$values['product_id']` / `$values['variation_id']` (the canonical, never-mutated values used to compute the cart line's KEY), NOT `$values['data']->get_id()` (which after the session-load filter returns the translated product's ID and would not match the stored KEY). This was caught during the staff-engineer pass after Pass-2 revert.
...
- **Store API `/cart/items` dedup coverage**: `POST /wc/store/v1/cart/items` is now covered by a `rest_request_before_callbacks` hook that rewrites request `id` to an existing cart line's canonical product/variation ID before `CartController::add_to_cart()` executes, so dedup behavior matches `/cart/add-item`.
- **Variation attribute slug sync**: when translating a variation cart item, `$cart_item['variation']` (the attribute_TAX → term_slug map) is replaced with the translated variation's own attribute map via `WC_Product_Variation::get_variation_attributes()`. Without this, `CartItemSchema::format_variation_data()` would resolve attribute terms via `get_term_by('slug', $source_lang_slug, ...)` and could leak source-language slug values to Block cart.
- **Block cart compatibility flag**: `cart_checkout_blocks` feature is declared `true`.
- **Existing v1.x render-time filters** (`woocommerce_cart_item_product`, `woocommerce_cart_item_product_id`, `woocommerce_cart_item_permalink`, `woocommerce_get_item_data`) kept as safety nets for classic-cart edge cases.
