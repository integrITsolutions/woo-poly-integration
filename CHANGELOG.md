...
- **Known limitation**: `POST /wc/store/v1/cart/items` (rare alternative endpoint per WC 10.7 `CartItems.php`) bypasses the `woocommerce_store_api_add_to_cart_data` filter; dedup does not run on that path. Block cart UI uses `/cart/add-item` which IS covered.
